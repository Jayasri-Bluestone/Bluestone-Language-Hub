    ...
    >
      <div className="sticky top-0 h-screen flex items-center overflow-hidden z-10">
        ...
        >
          ...
          {/* Feature Cards */}
          {items.map((item, i) => (
            <div key={i} className="flex-shrink-0 w-[450px] h-[500px] flex flex-col justify-end p-20 rounded-[80px] relative overflow-hidden group shadow-2xl transition-all duration-1000 hover:translate-y-[-20px] border border-gray-100">
              ...
              <div className="relative z-10 text-white">
                <span className="text-xs font-black uppercase tracking-[0.4em] mb-8 block text-brand-green-pop">Pillar 0{i+1}</span>
                <h3 className="text-7xl font-black mb-8 tracking-tighter leading-tight">{item.title}</h3>
                <p className="text-2xl opacity-70 leading-relaxed font-bold max-w-md">
                  {item.text}
                </p>
          ...
          <div className="flex-shrink-0 w-[50vw] flex flex-col justify-center items-center text-center">
             <motion.h2 
              whileInView={{ scale: [0.9, 1.1, 1], rotate: [0, 2, -2, 0] }}
              className="text-[100px] font-black text-gray-900 mb-16 leading-[1]"
             >
                Ready to <br /> <span className="text-gradient">LEAP?</span>
             </motion.h2>
             <div className="p-2 rounded-full bg-green-gradient shadow-[0_30px_100px_rgba(34,197,94,0.4)]" onClick={openContactModal}>
                <button className="px-16 py-6 bg-white text-brand-green rounded-full text-3xl font-black hover:bg-transparent hover:text-white transition-all transform hover:scale-105 active:scale-95 flex items-center gap-4">
